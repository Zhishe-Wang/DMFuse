...
class DDPM(BaseModel):
    def __init__(self, opt):
        ...
        self.netG = self.set_device(model.networks.define_G(opt))
    ...
    def get_train_ir(self):
        self.netG.eval()
        A = self.data["A"]
        return A

    def get_train_vi(self):
        self.netG.eval()
        B = self.data["B"]
        return B
    ...
    #     with torch.no_grad():
    #         if isinstance(self.netG, nn.DataParallel):
    #             ir1, ir2, ir3 = self.netG.module.feats(A, t)
    #             vi1, vi2, vi3 = self.netG.module.feats(B, t)
    #         else:
    #             ir1, ir2, ir3 = self.netG.feats(A, t)
    #             vi1, vi2, vi3 = self.netG.feats(B, t)
    #     self.netG.train()
    #     return ir1, ir2, ir3, vi1, vi2, vi3
    def get_feats(self, x, t):
        self.netG.eval()

        with torch.no_grad():
            if isinstance(self.netG, nn.DataParallel):
                en1, en2 = self.netG.module.feats(x, t)
            else:
                en1, en2 = self.netG.feats(x, t)
        self.netG.train()
        return en1, en2
    def get_x_t(self, x, t):
        self.netG.eval()
        ...
        with torch.no_grad():
            if isinstance(self.netG, nn.DataParallel):
                ...
                x_t = self.netG.x_noisy(x, t)
        self.netG.train()
        return x_t
    def get_recon(self, x, t):
        self.netG.eval()
